...
def test_transform_raw_data_cloud(settings_factory):
    ...
    config.raw_data_bucket = "raw"
    config.curated_data_bucket = "curated"
    config.raw_s3_prefix = "raw/"
    config.curated_s3_prefix = "curated/"
    ...
    mock_paginator.paginate.return_value = [{
        "Contents": [{"Key": "raw/FPT.json"}]
    }]
    
    # Mock get_object
    body = json.dumps([_entry("FPT", 11)]).encode("utf-8")
    mock_body = MagicMock()
