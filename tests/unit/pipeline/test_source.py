...
def test_yahoo_finance_source_success(monkeypatch):
    from unittest.mock import patch
    from src.pipeline.source import YahooFinanceSource
    import pandas as pd
...
def test_yahoo_finance_source_success(monkeypatch):
    ...
    with patch("requests.get", return_value=MockResponse()):
        source = YahooFinanceSource()
        df, attempts = source.history("FPT", "2023-01-01", "2023-01-02", "1D")
        assert not df.empty
        assert len(df) == 1
        assert df["close"].iloc[0] == 10.5

def test_yahoo_finance_source_fallback_yfinance():
    ...
    with patch("requests.get", return_value=MockResponse()):
        with patch.dict("sys.modules", {"yfinance": mock_yf}):
            source = YahooFinanceSource()
            df, _ = source.history("FPT", "2023-01-01", "2023-01-02", "1D")
            assert not df.empty